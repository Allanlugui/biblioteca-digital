...
export function ResultCard({ documento, voltar }: { documento: Documento; voltar: string }) {
  const ficha = `/documento/${encodeURIComponent(documento.id)}?voltar=${encodeURIComponent(voltar)}`;
  return (
    <article className="flex h-full flex-col rounded-md border border-rule bg-vellum p-6 shadow-[0_1px_0_rgba(34,26,16,0.06),0_10px_24px_rgba(34,26,16,0.05)] transition-shadow hover:shadow-[0_1px_0_rgba(34,26,16,0.06),0_16px_32px_rgba(34,26,16,0.10)]">
      ...
      <p className="mt-1 text-xs font-semibold uppercase tracking-wider text-ink-soft">
        ...
        {typeof documento.citacoes === "number" ? ` · ${documento.citacoes} ${documento.citacoes === 1 ? "citação" : "citações"}` : ""}
      </p>
      {documento.doi && <p className="mt-2 font-mono text-xs text-ink-soft">DOI: {documento.doi}</p>}
      {documento.assuntos.length > 0 && (
        <p className="mt-2 line-clamp-1 text-xs text-ink-soft">{documento.assuntos.slice(0, 3).join(" · ")}</p>
      )}
      {documento.descricao && <p className="mt-4 line-clamp-3 text-sm leading-6 text-ink-soft">{documento.descricao}</p>}
      <Link prefetch={false} href={ficha} className="mt-auto inline-flex items-center gap-2 pt-6 text-sm font-semibold text-library-800 underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-library-700">
