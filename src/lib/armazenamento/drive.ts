...
import { lerCredencialOAuth, obterAccessTokenOAuth } from "./google-auth";
import type { ArquivoBaixado, BackendArmazenamento } from "./tipos";

function configDrive(): { pastaId: string } | null {
  if (!lerCredencialOAuth() || !config.googleDriveFolderId) return null;
  return { pastaId: config.googleDriveFolderId };
}
...
async function buscarPorNome(pastaId: string, nome: string): Promise<string | null> {
  ...
  const resposta = await fetch(url, { headers: await cabecalhos() });
...
export function backendDrive(): BackendArmazenamento {  return {
    ...
    async guardar(path: string, bytes: ArrayBuffer, contentType: string): Promise<string> {
      ...
      const existente = await buscarPorNome(cfg.pastaId, path);
    ...
    async existe(path: string): Promise<boolean> {
      const cfg = configDrive();
      if (!cfg) return false;
      const id = path.startsWith("drive:") ? path.slice("drive:".length) : await buscarPorNome(cfg.pastaId, path);
      if (!id) return false;
